...
start_target_pos1 = (0, 0.65, 0.15)
start_target_pos2 = (0, -0.65, 0.15)
...
class PickPlaceRandomBlockEnv:
    def __init__(self, vis, device, num_envs=1):
        ...
        self.target1 = self.scene.add_entity(
            gs.morphs.Box(
                size=target_size,
                pos=start_target_pos1,
                fixed=True
            )
        )
        self.target2 = self.scene.add_entity(
            gs.morphs.Box(
                size=target_size,
                pos=start_target_pos2,
                fixed=True
            )
        )
    ...
    def reset(self):
        ...
        obs2 = self.target2.get_pos().to(dtype=torch.float64)
    ...
    def in_place_box(self, target_position2, hitbox_range_xy=0.02):
        cube_pos = self.cube.get_pos().to(dtype=torch.float64)
        # Check if the cube position is within the target hitbox
        lower_bound_xy = target_position2[:, :2] - hitbox_range_xy
        upper_bound_xy = target_position2[:, :2] + hitbox_range_xy
    ...
    def step(self, actions):
        ...
        target_position1 = self.target1.get_pos().to(dtype=torch.float64)
        target_position2 = self.target2.get_pos().to(dtype=torch.float64)
        ...
        action_mask_3 = actions == 3  # Move above target 1 fingers closed
        action_mask_4 = actions == 4  # Move on target 1
        action_mask_5 = actions == 5  # Move above target 1 fingers opened
        action_mask_6 = actions == 6  # Move above target 2 fingers closed
        action_mask_7 = actions == 7  # Move on target 2
        action_mask_8 = actions == 8  # Move above target 2 fingers opened
        ...
        pos[action_mask_3] = target_position1[action_mask_3] + hover_offset
        pos[action_mask_4] = target_position1[action_mask_4] + target_offset
        pos[action_mask_5] = target_position1[action_mask_5] + hover_offset
        pos[action_mask_6] = target_position2[action_mask_6] + hover_offset
        pos[action_mask_7] = target_position2[action_mask_7] + target_offset
        pos[action_mask_8] = target_position2[action_mask_8] + hover_offset
        ...
        qpos[action_mask_6] = target_qpos[action_mask_6]
        qpos[action_mask_7] = target_qpos[action_mask_7]
        qpos[action_mask_8] = target_qpos[action_mask_8]
        ...
        finger_positions[action_mask_6] = torch.tensor([0.0, 0.0], device=self.device, dtype=torch.float64)
        finger_positions[action_mask_7] = torch.tensor([0.0, 0.0], device=self.device, dtype=torch.float64)
        finger_positions[action_mask_8] = torch.tensor([0.4, 0.4], device=self.device, dtype=torch.float64)
        ...
        states = torch.concat([block_position, target_position2, gripper_position], dim=1)    

        # Cube distance from target
        cdft = torch.norm((target_position2 + target_offset) - block_position, dim=1)
        ...
        rewards = 1/(cdft + 1)*10
        dones = self.in_place_box(target_position2) & cube_at_rest
        rewards -= 0.5
        rewards[dones] += 100
